...
class MaquinaTuring:
    ...
    def __init__(self, cinta1, cinta2, operacion):
        self.cabezal1 = Cinta(len(cinta1))
        self.cabezal2 = Cinta(len(cinta2))
        self.cabezalResultado = Cinta(self.RESULT_TAPE_SIZE)
        self.cabezal1.cinta = cinta1[:]
        self.cabezal2.cinta = cinta2[:]
    ...
    def transicion(self):
        if self.estado == Estado.Q0:
            ...
            if self.cabezal1.cabezal >= 0 or self.cabezal2.cabezal >= 0 or self.acarreo > 0:
                self.estado = Estado.Q2
            else:
                self.estado = Estado.Q5

        elif self.estado == Estado.Q2:
            bit1 = self.cabezal1.leer()
            bit2 = self.cabezal2.leer()

            if self.operacion == '+':
                suma = bit1 + bit2 + self.acarreo
                if suma == 0 or suma == 1:
                    self.cabezalResultado.escribir(suma)
                    self.acarreo = 0
                elif suma == 2:
                    self.cabezalResultado.escribir(0)
                    self.acarreo = 1
                elif suma == 3:
                    self.cabezalResultado.escribir(1)
                    self.acarreo = 1

            elif self.operacion == '-':
                bit1 -= self.prestamo
                if bit1 >= bit2:
                    self.cabezalResultado.escribir(bit1 - bit2)
                    self.prestamo = 0
                else:
                    self.cabezalResultado.escribir((bit1 + 2) - bit2)
                    self.prestamo = 1
            ...
            self.cabezal1.mover_izquierda()
            self.cabezal2.mover_izquierda()
            self.cabezalResultado.mover_izquierda()
            self.estado = Estado.Q1
    ...
    def imprimir_resultado(self):
        return self.cabezalResultado.imprimir()
