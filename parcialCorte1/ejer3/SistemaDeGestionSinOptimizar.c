...
// Definición de la estructura básica (sin optimización) para almacenar la información del estudiante
typedef struct {
    char *nombre;
    char *apellido;
    unsigned int edad;  // Almacenado con un entero completo
    char *ID;           // ID almacenado como cadena de texto dinámica
    int *calificaciones;
...
// Función para calcular el uso de memoria (sin optimización)
size_t calcular_memoria_utilizada(const SistemaEstudiantes *sistema) {
    ...
    for (size_t i = 0; i < sistema->num_estudiantes; i++) {
        ...
        memoria_total += strlen(estudiante->ID) + 1;  // Memoria para el ID
...
void actualizar_estudiante(SistemaEstudiantes *sistema, const char *ID, const char *nuevo_nombre, const char *nuevo_apellido, unsigned int nueva_edad) {
    ...
    estudiante->ID = (char *)malloc(strlen(ID) + 1);
...
int main() {
    ...
    // Liberar todos los estudiantes al final
